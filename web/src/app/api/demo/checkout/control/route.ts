...
import { healthyControlCheckoutBody } from "../../../../../features/demo/trigger";
...
export async function POST(): Promise<Response> {
  ...
  try {
    const upstream = await fetch(gatewayUrl, {
      ...
      body: JSON.stringify(healthyControlCheckoutBody),
    });
    if (!upstream.ok) {
      return frozenError(502, "The gateway rejected the healthy control checkout trigger.");
    }
    ...
    if (typeof trace?.trace_id !== "string" || typeof trace?.execution_id !== "string") {
      return frozenError(502, "The gateway returned an unusable healthy control response.");
    }
